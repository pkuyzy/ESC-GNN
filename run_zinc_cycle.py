...
import torch.nn
from dataset_zinc_cycle import ZINC
import argparse
from tqdm import tqdm
# from k_gnn import TwoMalkin, ConnectedThreeMalkin, TwoLocal, ThreeMalkin, ThreeLocal
from zinc_cycle_models import *
from utils import create_subgraphs
from utils_edge_efficient import create_subgraphs as create_subgraphs_eff
from utils_edge_I2 import create_subgraphs2
import numpy as np
import random
...
# Base GNN settings.
parser.add_argument('--model', type=str, default='NestedGIN_eff')#NestedGIN_eff', help = 'GNN/NestedGIN_eff/I2GNN')
...
# Backup python files.
copy('run_zinc_cycle.py', args.res_dir)
copy('utils.py', args.res_dir)
copy('zinc_cycle_models.py', args.res_dir)
...
if args.h is not None:
    ...
    def pre_transform2(g):
        return  create_subgraphs_eff(g, args.h,
                                 max_nodes_per_hop=args.max_nodes_per_hop,
                                 use_rd=args.use_rd,
                                 subgraph_pretransform=subgraph_pretransform,
                                 self_loop=args.self_loop)
    def pre_transform3(g):
...
elif args.model == 'NestedGIN_eff':
    processed_name = 'processed_eff_h'+str(args.h)+"_"+args.node_label
    if args.h is not None:
        my_pre_transform = pre_transform2
    else:
        my_pre_transform = None
elif args.model == 'I2GNN':
    processed_name = 'processed_I2_h'+str(args.h)+"_"+args.node_label
    my_pre_transform = pre_transform3
else:
...
if args.model == 'I2GNN' or args.model == 'NGNN':
    from dataloader_I2 import DataLoader  # use a custom dataloader to handle subgraphs
else:
    from dataloader import DataLoader  # use a custom dataloader to handle subgraphs

data_path = './data/zinc_cycle'
...
t12 = time.time()
print("Preprocessing time cost: {}s,".format(t12 - t11))

# for dataset in [train_dataset, test_dataset, val_dataset]:
    ## choice 1: drop second identifier
    #dataset.data.z[:, 2:] = torch.zeros_like(dataset.data.z[:, 2:])
    #dataset.data.rd[:, 1] = torch.zeros_like(dataset.data.rd[:, 0])
    #dataset.data.center_idx[:, -1] = dataset.data.center_idx[:, 0]

    ## choice 1: drop all identifiers
    # dataset.data.rd = torch.zeros_like(dataset.data.rd)
    # dataset.data.z = torch.zeros_like(dataset.data.z)
...
target = args.target
y_train_val = torch.cat([train_dataset.data.y[:, target], val_dataset.data.y[:, target]], dim=0)
mean = y_train_val.mean(dim=0)
std = y_train_val.std(dim=0)
#train_dataset.data.y[:, target] = (train_dataset.data.y[:, target] - mean) / std
#val_dataset.data.y[:, target] = (val_dataset.data.y[:, target] - mean) / std
...
def loop(start=1, best_val_error=None):
    ...
    start = epoch + 1
    t2 = time.time()
    print("Preprocessing time cost: {}s, Training time cost: {}s".format(t12 - t11, t2 -t1))
