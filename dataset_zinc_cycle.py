...
class ZINC(InMemoryDataset):
    def __init__(self, root='data/zinc_cycle', processed_name='processed', dataset='train',
    ...
    def pkl2data(self, d):
        ...
        G = to_networkx(Data(num_nodes=len(x), edge_index=edge_index))
        
        cycles = torch.zeros([len(x), 4]).float()
        sc = [s for s in sorted(nx.simple_cycles(G)) if len(s) >= 3 and len(s) <= 6]
        for s in sc:
            cycles[s, len(s) - 3] += 1
        
        assert x.size(0) == torch.max(edge_index) + 1
        return Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y=cycles/2)
