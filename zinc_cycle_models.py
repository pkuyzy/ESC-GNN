...
from torch.nn import ModuleList, ReLU, Sequential, ELU
...
class I2GNN(torch.nn.Module):
    ...
    def graph_pooling(self, x, data, z=None, layer=None, aggr='mean', node_emb_only=False):
        ...
        if self.subgraph2_pooling == 'mean':
            ...
            #x = torch.cat([x[data.center_idx[:, 0]],
            #               x[data.center_idx[:, 1]]], dim=-1)
    ...
    def forward(self, data):
        ...
        for layer, conv in enumerate(self.convs):
            ...
            if self.double_pooling:
                x = torch.cat(
                    [x, self.graph_pooling(x, data, z_emb, layer, node_emb_only=True)[data.node_to_original_node]],
                    dim=-1)
        ...
        x = self.graph_pooling(x, data, z_emb, -1, aggr = 'no')
...
class NGNN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        # subgraph-level pooling
        if self.subgraph_pooling == 'center':
...
class NestedGIN_eff(torch.nn.Module):
    def __init__(self, dataset, num_layers, concat=False, use_pos = False, use_max_dist=False,RNI=False,**kwargs):
        ...
        input_dim = 32#dataset.num_features
        ...
        self.node_type_embedding = torch.nn.Embedding(100, 32)
        self.edge_type_embedding = torch.nn.Embedding(100, 32)
    ...
    def forward(self, data):
        ...
        x, edge_index, batch = self.node_type_embedding(data.x), data.edge_index, data.batch
        ...
        z_emb = torch.cat((z_emb, self.edge_type_embedding(data.edge_attr)), dim = -1)
        ...
        x = self.lin1(torch.cat(xs, dim=1))
        if x.size()[0] > 1:
            x = self.bn_lin1(x)
