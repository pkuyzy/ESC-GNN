...
from graphgps.loader.batch import Batch
...
def create_subgraphs(data, h=1, sample_ratio=1.0, max_nodes_per_hop=None,
    ...
    SPT = dict(nx.all_pairs_shortest_path_length(G))
    attn_bias = torch.zeros(len(SPT), len(SPT), dtype = torch.long)
    for i in range(data.num_nodes):
        for j in range(data.num_nodes):
            if j in SPT[i]:
                attn_bias[i][j] = SPT[i][j]
            else:
                attn_bias[i][j] = 100
    attn_bias = attn_bias.reshape(-1)
    ...
    for h_ in h:
        ...
        if not hasattr(data, 'pos'):
            new_data =  data.__class__(data.x, edge_index, edge_attr, data.y, None, pos_enc = torch.cat(pos_encs, dim = 0), pos_index = torch.cat(pos_indices, dim = 0), pos_batch = torch.cat(pos_batches, dim = 0), attn_bias = attn_bias)
        elif not hasattr(data, 'name'):
            new_data= data.__class__(data.x, edge_index, edge_attr, data.y, None, pos_enc = torch.cat(pos_encs, dim = 0), pos_index = torch.cat(pos_indices, dim = 0), pos_batch = torch.cat(pos_batches, dim = 0), attn_bias = attn_bias)
        else:
            new_data = data.__class__(data.x, edge_index, edge_attr, data.y, pos = data.pos, pos_enc = torch.cat(pos_encs, dim = 0), pos_index = torch.cat(pos_indices, dim = 0), pos_batch = torch.cat(pos_batches, dim = 0), name = data.name, node_type = data.node_type, attn_bias = attn_bias)
    return new_data
    '''
