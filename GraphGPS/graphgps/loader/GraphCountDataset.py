...
class dataset_random_graph(InMemoryDataset):
    ...
    def adj2data(self, A, y):
        ...
        if y.ndim == 1:
            y = y.reshape([1, -1])
        return Data(x = torch.ones(num_nodes).view(num_nodes, 1).repeat(1, 10), edge_index=edge_index, edge_attr = torch.ones(edge_index.size(1), 1), y=torch.tensor(y), num_nodes=torch.tensor([num_nodes]))
    ...
    def process(self):
        ...
        for save_path, data_list in zip(self.processed_paths, data_list_all):
            ...
            if self.pre_transform is not None:
                temp = []
                for i, data in enumerate(data_list[0:100]):
                    if i % 100 == 0:
