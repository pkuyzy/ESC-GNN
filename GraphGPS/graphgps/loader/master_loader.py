...
from torch_geometric.datasets import (Actor, GNNBenchmarkDataset, Planetoid,
                                      TUDataset, WebKB, WikipediaNetwork, QM9)
from graphgps.loader.zinc import ZINC
...
from graphgps.transform.transforms import (pre_transform_in_memory,
                                           ...
                                           clip_graphs_to_size)
from graphgps.loader.utils_escgnn import create_subgraphs
from graphgps.loader.GraphCountDataset import dataset_random_graph

def pre_transform2(g, hop = 3):
    return  create_subgraphs(g, hop, use_rd=True, self_loop=True)
...
class MyTransform(object):
    ...
    def __call__(self, data):
        #data.y = data.y[:, self.target]  # Specify target: 0 = mu
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        if pyg_dataset_id.startswith('QM9') or pyg_dataset_id.startswith('Count'):
            pyg_dataset_id = format.split('-')[1]
            target = int(format.split('-')[2])
        ...
        if pyg_dataset_id == 'Actor':
            ...
            dataset = QM9(dataset_dir, transform = MyTransform(target = target))
            dataset.data.y = dataset.data.y[:, target]
            dataset.data.x = torch.cat((dataset.data.x, dataset.data.pos), dim = 1)
            tenpercent = int(len(dataset) * 0.1)
            mean = dataset.data.y[tenpercent:].mean(dim=0)
            std = dataset.data.y[tenpercent:].std(dim=0)
            dataset.data.y = (dataset.data.y - mean) / std
            dataset = join_dataset_splits([dataset[2 * tenpercent:], dataset[tenpercent:2 * tenpercent], dataset[:tenpercent]])

        elif pyg_dataset_id == 'Count':
            count_dataset = 'count_graphlet' if target > 3 else 'count_cycle'
            
            train_dataset = dataset_random_graph(dataname=count_dataset, split='train')
            val_dataset = dataset_random_graph(dataname=count_dataset, split='val')
            test_dataset = dataset_random_graph(dataname=count_dataset, split='test')
            
            
            target_final = target if target <= 3 else target - 4
            train_dataset.data.y = train_dataset.data.y[:, target_final]
            val_dataset.data.y = val_dataset.data.y[:, target_final]
            test_dataset.data.y = test_dataset.data.y[:, target_final]
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == 'ogbg-molhiv':
        ...
        hop = 3
    def pre_transformogb(g):
        return  create_subgraphs(g, hop, use_rd=True, self_loop=True)
    #dataset = PygGraphPropPredDataset(name=name, root=dataset_dir)
    dataset = PygGraphPropPredDataset(name=name, root=dataset_dir, pre_transform = pre_transformogb)
...
def preformat_ZINC(dataset_dir, name):
    ...
    dataset = join_dataset_splits(
        ...
        [ZINC(root=dataset_dir, subset=(name == 'subset'), split=split, pre_transform = pre_transform2)
         #for split in ['train', 'val', 'test']]
         for split in ['train', 'val', 'test']]
    )
    return dataset
