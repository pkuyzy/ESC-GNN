...
#from graphgps.layer.batch import Batch
...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        self.edge_attn_emb = torch.nn.Embedding(101, num_heads, padding_idx = 100)
        self.edge_attn_linear = torch.nn.Sequential(
                torch.nn.Linear(num_heads, num_heads),
                #torch.nn.BatchNorm2d(num_heads),
                torch.nn.ReLU(),
                torch.nn.Linear(num_heads, num_heads),
                #torch.nn.BatchNorm2d(num_heads),
                torch.nn.ReLU(),
        ...
        hidden = dim_h
        self.z_initial = nn.Embedding(z_in, hidden)
    ...
    def forward(self, batch):
        if hasattr(batch, 'pos_index'):
            z_emb = global_add_pool(torch.mul(self.z_initial.weight[batch.pos_index], batch.pos_enc.view(-1, 1)), batch.pos_batch)
            #batch.edge_attr += z_emb
            batch.edge_attr += self.z_embedding(z_emb)
    ...
    def _sa_block(self, x, attn_mask, key_padding_mask):
        ...
        if attn_mask is not None:
            ...
            attn_mask = self.edge_attn_emb(attn_mask)
            num_node = attn_mask.size(1)
            attn_mask = self.edge_attn_linear(attn_mask)
            attn_mask = attn_mask.permute(0, 3, 1, 2).reshape(-1, num_node, num_node)
