...
def create_subgraphs(data, h=1, sample_ratio=1.0, max_nodes_per_hop=None,
                     node_label='hop', use_rd=False, subgraph_pretransform=None, data_name = None):
    ...
    for h_ in h:
        ...
        new_data.node_id = torch.LongTensor([0] + [i for i in range(1, len(new_data.batch)) if new_data.batch[i] != new_data.batch[i - 1]])
        '''
        if data_name is not None:
            try:
                new_data.cycles = torch.load("/data1/count_cycles/" + data_name + ".pt")
                new_data.cycles[new_data.cycles != 0] = 1
            except BaseException:
                print(data_name + " no cycles loaded")
        '''
        del new_data.batch
